...
The start type of the **`SEMgrSvc`** service - Payments and NFC/SE Manager - from `Manual` to
`Manual`, or from `Automatic` if something has changed it, and stops the running instance.

## Mechanism

`SEMgrSvc` manages the **secure element**: the tamper-resistant chip used for tap-to-pay and
similar NFC transactions. It is a phone and tablet feature that came along with the shared
Windows codebase.

A desktop PC does not have a secure element. Neither do the overwhelming majority of laptops. On
those machines this is a service managing hardware that is not present.

## How much this is worth, honestly

No measured benefit, and none likely: the service is already `Manual` on a default install and
does not start unless something asks for NFC payments, which nothing on a desktop ever does.
Filed under **Background & Cleanup** with the rest of the background-service family.

Applying this on a default Windows 11 desktop is very close to a no-op. It is in the catalog for
completeness and so that the value is pinned and watched.

## Trade-off

If your machine does have NFC payment hardware and you use it, this breaks it. Otherwise nothing.
