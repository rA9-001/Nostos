...
The start type of the **`XblAuthManager`** service - Xbox Live Auth Manager - from `Manual` to
`Manual` (usually already the case) and stops the running instance. Where the machine has it on
`Automatic`, this moves it off.

## Mechanism

`XblAuthManager` signs the machine in to Xbox Live and issues the tokens that Microsoft Store
and Game Pass titles use to authenticate. Anything from the Xbox app, and a number of Microsoft
published games sold on Steam, calls into it.

A library that is entirely Steam, Epic or GOG never touches it.

## How much this is worth, honestly

No measured benefit. The service is demand-started, idles at nothing, and the claim that turning
it off buys frames is unsupported. Filed under **Background & Cleanup** with its neighbours because
that is where the "background services cause hitching" claim lives, and this page marks it
unproven.

## Trade-off

**Game Pass and Microsoft Store games stop launching.** So do a handful of Microsoft-published
Steam titles that still require an Xbox Live sign-in - *Sea of Thieves*, *Forza*, *Halo*, *Age
of Empires* and others - which is the case people forget, because they think of those as Steam
games.

On `manual` the service starts on demand when a title asks for a token, so in practice this is
recoverable. On `disabled` the sign-in fails and the game shows a generic network error.
