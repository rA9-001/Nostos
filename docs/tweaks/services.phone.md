...
The start type of the **`PhoneSvc`** service - Phone Service - from `Automatic` to `Manual`, or from
...
`PhoneSvc` manages the telephony state of the device itself: cellular calls and call state on
a Windows tablet or laptop with a built-in modem.

## How much this is worth, honestly

It is worth being clear about what this is not. **Phone Link**, the app that mirrors your
Android phone's messages and calls to the desktop, is a separate Store app with its own
background process, and it keeps working with this service stopped.

`PhoneSvc` is for a modem in *this* machine. If there is not one, the service manages nothing.

## Trade-off

Cellular calling on a machine with a built-in modem stops working. On a desktop there is
nothing to lose.
