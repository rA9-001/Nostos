...
The start type of the **`Fax`** service from `Manual` (its usual state) to `Manual` - so on most
machines this reads as applied already - or from `Automatic` to `Manual` if something has changed
it, and stops the running instance.

The service does not exist on every edition. Where it is absent the tweak reports itself as not
applicable rather than failing.

## Mechanism

The Fax service sends and receives faxes through a fax modem. If your PC does not have a fax
modem attached - and it does not - this service has no hardware to talk to.

## How much this is worth, honestly

Same as its neighbours: the performance claim is unsupported, because a service sitting at
`Manual` and not running consumes nothing. This entry is filed under **Background & Cleanup** with
the rest of the background-service family, and it is here for completeness and for pinning
rather than because it will do anything for your framerate.

If you want the honest version: applying this on a default Windows 11 machine changes nothing at
all. It is in the catalog so that a machine somebody else has been through can be checked and
put back.

## Trade-off

You cannot send or receive faxes. Windows Fax and Scan's scanning half is a different component
and keeps working.
