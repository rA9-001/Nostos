...
The start type of the **`SDRSVC`** service - Windows Backup - from `Automatic` to `Manual`, or from
...
`SDRSVC` runs the Backup and Restore (Windows 7) engine - the file-and-image backup that
predates File History and is still shipped.

## How much this is worth, honestly

When a scheduled backup fires it reads a large fraction of the disk and writes it somewhere
else, which is the single heaviest piece of background I/O Windows will start on its own. That
is a real interruption if it lands mid-session.

The catch is that it only fires if you configured a schedule, and most people never did. On a
machine with no backup set up this service is inert and stopping it buys nothing.

**Do not apply this if it is your backup.** A backup that silently stops running is worse than
no backup, because you will not find out until you need it.

## Trade-off

Scheduled Backup and Restore (Windows 7) jobs stop running. System image creation stops
working.

File History is a different service and is unaffected, as is any third-party backup tool.
